...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    remainingCorners = [corner for i, corner in enumerate(corners) if not visited[i]]
    ...
    # Lower bound from a relaxed problem: ignore walls and take the best
    # Manhattan-distance tour that visits all remaining corners.
    from itertools import permutations
    best = float("inf")
    for order in permutations(remainingCorners):
        cost = manhattan(position, order[0])
        for i in range(len(order) - 1):
            cost += manhattan(order[i], order[i + 1])
        best = min(best, cost)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    # Lower bound 1: must at least reach one of the remaining food dots.
    farthestFromPacman = max(maze_dist(position, food) for food in foodList)

    # Lower bound 2: path that eats all food must connect the two farthest food dots.
    foodDiameter = 0
    for i in range(len(foodList)):
        for j in range(i + 1, len(foodList)):
            foodDiameter = max(foodDiameter, maze_dist(foodList[i], foodList[j]))

    return max(farthestFromPacman, foodDiameter)
